...
export class HospitableAPI {
  ...
  async getCustomerListings(customerId: string): Promise<Property[]> {
    ...
    const response = await this.request<Property[]>(
      `/api/v1/customers/${customerId}/listings`
    );
    return response.data;
  }
  ...
  async getBookings(filters?: {
    ...
    const response = await this.request<Booking[]>(
  ...
  async markListingsForPublishing(
    ...
    const response = await fetch("/api/hospitable/publish-properties", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(
        error.message || "Failed to mark properties for publishing"
      );
    }
  ...
  async getPropertyImages(
    ...
    const response = await this.request<PropertyImage[]>(
      `/api/v1/customers/${customerId}/listings/${listingId}/images`
    );
    return response.data;
  }

  // Get a single property image for a specific customer and listing - useful for getting the main image
  async getPropertyMainImage(
    customerId: string,
    listingId: string
