...
// Extend session type to include hospitableToken
...
export async function getOrRefreshHospitableToken(
  req: Request
): Promise<string | null> {
  try {
    // Check if we have an access token in the session
    if (
      req.session?.hospitableToken?.access_token &&
      req.session?.hospitableToken?.expires_at
    ) {
      ...
      if (req.session.hospitableToken.refresh_token) {
        const refreshedToken = await refreshHospitableToken(
          req.session.hospitableToken.refresh_token
        );
...
async function refreshHospitableToken(refreshToken: string) {
  try {
    ...
    const response = await axios.post(
      "https://connect.hospitable.com/oauth/token",
      {
        client_id: clientId,
