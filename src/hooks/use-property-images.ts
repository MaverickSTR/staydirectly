...
export function usePropertyImages(
  ...
  storedImages?: { mainImage?: string; additionalImages?: string[] } | null
) {
  // Extract customer ID and listing ID from the platformId
  const { customerId: extractedCustomerId, listingId } =
    extractPropertyIds(platformId);
  ...
  const hasAdditionalImages = !!(
    storedImages?.additionalImages && storedImages.additionalImages.length > 0
  );
  ...
  return useQuery<PropertyImage[]>({
    ...
    queryFn: async () => {
      ...
      console.log(
        `Fetching from API: customerId=${customerId}, listingId=${listingId}`
      );
...
export function usePropertyMainImage(
  ...
  return useQuery<PropertyImage | undefined>({
    ...
    queryFn: async (): Promise<PropertyImage | undefined> => {
      ...
      const imageUrl = await fetchPropertyMainImage(customerId, listingId);
      if (!imageUrl) {
        return undefined;
      }
    ...
    enabled:
      (!storedMainImage && !!customerId && !!listingId) || !!storedMainImage,
 ...
 */
export function useRefreshPropertyImages(
  propertyId: number,
  platformId: string | null,
  defaultCustomerId?: string
) {
  ...
  return useQuery<{ success: boolean; message: string }>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const mainImage =
          images.find((img: PropertyImage) => img.isPrimary) || images[0];
        const additionalImages = images
          .filter((img: PropertyImage) => img.id !== mainImage.id)
          .sort(
